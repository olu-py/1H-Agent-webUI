...
# Usage: AGENT_SMOKE_BIN=/path/to/1h-agent-web bash scripts/smoke-web.sh

set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BIN="${AGENT_SMOKE_BIN:-$ROOT/target/debug/1h-agent-web}"
...
CURL=(curl --silent --show-error --connect-timeout 2 --max-time 5)

# status <name> <expected> <curl args...>
status() {
  local name="$1" want="$2"; shift 2
  local got; got=$("${CURL[@]}" -o /dev/null -w "%{http_code}" "$@")
...
if [ -n "${AGENT_SMOKE_BIN:-}" ] && [ ! -x "$BIN" ]; then
  echo "AGENT_SMOKE_BIN is not executable: $BIN"
  exit 1
elif [ ! -x "$BIN" ]; then
...
ready=false
for _ in $(seq 1 30); do
  if "${CURL[@]}" --max-time 1 -o /dev/null "$BASE/api/v2/state" 2>/dev/null; then
    ready=true
    break
  fi
  if ! kill -0 "$SRV_PID" 2>/dev/null; then
...
JS=$("${CURL[@]}" "$BASE/" | grep -o 'assets/index-[^"]*\.js' | head -1)
if [ -n "$JS" ] && "${CURL[@]}" -o /dev/null -w "%{http_code}" "$BASE/$JS" | grep -q 200; then
...
if "${CURL[@]}" "$BASE/api/v2/state" | grep -q '"protocol_version":2'; then
...
echo "== SSE =="
if "${CURL[@]}" --no-buffer --max-time 1 -D - -o /dev/null "$BASE/api/v2/events?cursor=0" 2>/dev/null | grep -qi "text/event-stream"; then
