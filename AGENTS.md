...
migration: 多界面低耦合改造已完成并归档（design/webui-migration.md，本地留存）；design/ 仅存放任务期间产生、完成后归档或删除的临时维护计划和实施记录（git 忽略不上传）；长期有效的维护指南与 workflow 必须放入 .agents/guides/ 并登记在本文件路由表
```
