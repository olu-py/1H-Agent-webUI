...
- 下行走 SSE：全局有界回放环 `EventBridge`（进程内单调游标、容量 clamp）；客户端从快照 `event_cursor` 订阅（先 replay 后 subscribe，防丢/重），游标被逐出或滞后发 `resync_required`。
...
- 前端：`cd web && pnpm install --frozen-lockfile && pnpm typecheck && pnpm test && pnpm build`；改动 `web/dist` 后提交（内嵌产物）。
